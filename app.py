...
st.set_page_config(page_title="Smart Health Diary V2.4", page_icon="🥗", layout="wide")
...
try:
    ...
    # เพิ่ม ttl=0 เพื่อบังคับให้แอปดึงข้อมูลใหม่สดๆ ทุกครั้ง ไม่แอบจำของเก่า
...
if st.button("🟢 💾 บันทึกเฉพาะมื้ออาหาร", use_container_width=True, type="primary"):
    with st.spinner("กำลังบันทึกมื้ออาหาร..."):
        ...
        # ล้างแคชและรีเซ็ตค่ากลับไปเป็น Default
        st.cache_data.clear()
...
# แปลงวันที่ให้ชัวร์ และตัดข้อมูลที่ไม่ใช่วันที่ออก
...
if df.empty:
    ...
    view_mode = st.radio("📍 2. รูปแบบการแสดงผล:", ["📊 ดูภาพรวมทั้งเดือน", "📅 ดูข้อมูลรายวัน"], horizontal=True)

    if view_mode == "📊 ดูภาพรวมทั้งเดือน":
        ...
        sel_date = st.selectbox("🎯 3. เลือกวันที่", days_in_month)
        filtered_df = df_month[df_month['Date_Str'] == sel_date]
        st.info(f"**กำลังแสดงสถิติประจำวันที่:** {sel_date}")
